 ...
 * - 힙 할당 최소화 (GC-free 수준)
 */
final class PlayerData implements Serializable {
    ...
    boolean isActive(String qid) {
        Node n = map.get(qid);
        return n != null && n.active;
    }

    boolean isCompleted(String qid) {
        Node n = map.get(qid);
        return n != null && n.completed;
    }

    void start(String qid) {
        qid = qid.intern();
    ...
    void complete(String qid, int pts) {
        Node n = map.computeIfAbsent(qid, k -> new Node());
        n.active = false;
        n.completed = true;
        n.points += pts;
    ...
    int add(String qid, int amt) {
        Node n = map.computeIfAbsent(qid, k -> new Node());
        n.value += amt;
        return n.value;
    }

    int valueOf(String qid) {
        Node n = map.get(qid);
        return (n == null) ? 0 : n.value;
    }

    /** QuestRepository.save() 에서 사용됨 */
    int pointsOf(String qid) {
        Node n = map.get(qid);
